...
module.exports = knex => {
  ...
  linksRoutes.post("/create", (req, res) => {
    ...
    knex
      .select("id")
      .from("topics")
      .where("name", req.body.link_topic)
      .then(function(result) {
        const topic = result[0].id;
        knex
          .select("id")
          .from("boards")
          .where("title", req.body.link_board)
          .then(function(result) {
            ...
            knex
              ...
              .then(function(result) {
                const id = result[0];
                knex
                  .insert({ link_id: id, userid: userid })
                  .into("learnt_counters")
                  .then(result => {
                    return knex
                      .insert({ link_id: id, board_id: board })
                      .into("boards_links");
                  })
  ...
  linksRoutes.get("/:linkId", (req, res) => {
    ...
    knex
      .select("*", "links.create_date AS create_date")
      .from("links")
                ...
                knex
                  ...
                  .then(function(result) {
                    ...
                    knex
                      ...
                      .where({ link_id: linkId, userid: req.session.userid })
                      .then(function(result) {
                        if (!result[0]) {
                          const vartemplate = {
                            id: req.session.userid,
  ...
  //submit edit
  linksRoutes.put("/:linkId/edit", (req, res) => {
    ...
    knex
      ...
      .then(gettopic => {
        knex("links")
          .where({ id: link_id })
          .update({
            topic_id: gettopic[0].id,
            title: req.body.link_title,
            description: req.body.link_desc
          })
          .then(result => {
  ...
  //delete link
  linksRoutes.delete("/:linkId/delete", (req, res) => {
    const currentUser = req.session.userid;
    const link_id = req.params.linkId;
    knex
      .select("user_id")
  ...
  linksRoutes.get("/:linkId/comments", (req, res) => {
    const linkId = req.params.linkId;
    knex
      .select("*", "comments.create_date as create_date")
      .from("comments")
    ...
    knex
      ...
      .then(result => {
        ...
        knex
          ...
          .andWhere("board_id", boardID)
          .then(result => {
            if (result[0] === undefined) {
              knex
                .insert({ link_id: link_id, board_id: boardID })
                .into("boards_links")
                .asCallback(function(err) {
                  if (err) {
                    return res.status(500).json({ error: err.message });
                  } else {
                    return res.redirect(`/links/${link_id}/`);
                  }
                });
            } else {
              return res.redirect(`/links/${link_id}/`);
  ...
  linksRoutes.post("/:linkId/learnt", (req, res) => {
    ...
    knex
      .select("learnt")
      .from("learnt_counters")
      .where({ link_id: linkid, userid: userID })
      .then(result => {
        if (result[0] === undefined) {
          knex("learnt_counters")
            .insert({ link_id: linkid, userid: userID, learnt: 1 })
            .then(() => {
              return res.redirect(`/links/${linkid}/`);
            });
        } else if (result[0].learnt == 0) {
          knex("learnt_counters")
            .where({ link_id: linkid, userid: userID })
            .increment("learnt", 1)
            .asCallback(function(err) {
